...
from keras.layers import Layer, Ones, Zeros,  Activation, Dropout, Lambda, Add, Dense, Input, Conv1D, TimeDistributed, Concatenate, Embedding, SeparableConv1D
...
class InferRNN(Layer):
	...
	def call(self, inputs, mask=None, training=None, initial_state=None, constants=None):
		...
		last_output, outputs, _ = K.rnn(step, inputs, initial_state, constants=constants,
...
class Transformer:
	def __init__(self, i_tokens, o_tokens, len_limit, d_model=256, \
		...
		self.i_word_emb = Embedding(i_tokens.length, d_emb)
		if share_word_emb: 
			assert i_tokens.length == o_tokens.length
			self.o_word_emb = self.i_word_emb
		else: self.o_word_emb = Embedding(o_tokens.length, d_emb)
		...
		self.target_layer = TimeDistributed(Dense(o_tokens.length, use_bias=False))
